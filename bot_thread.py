...
def mask_sensitive_data(text: str) -> str:
    ...
    if cmd in ["/set_pin", "/reset_pin", "/admin_reset_pin"] and len(words) >= 2:
        return f"{cmd} ***"
    ...
    def run(self):
        ...
        async def admin_panel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if user_lang == 'en':
                admin_panel_card = (
                    ...
                    "• `/admin_license 123456789 30` - Grant or revoke VIP membership\n"
                    "• `/admin_users` - View full registered user directory & status\n\n"
                    "🚨 **3. Signal & Emergency Operations:**\n"
                    "• `/admin_signal BTC BUY 10` - Broadcast auto-entry trade signal\n"
                    "• `/admin_broadcast Market update` - Send instant alert to all registered users\n"
                    "• `/toggle_breaker` - Toggle Emergency Circuit Breaker on/off\n"
                    "• `/toggle_rebalance` - Toggle Smart Capital Rebalance on/off\n"
                    "• `/admin_reset_pin 123456789` - Reset user 2FA PIN code\n"
                    "• `/admin_delete 123456789` - Delete user account registry\n"
                    "• `/admin_nuke 1234` - Emergency Panic Nuke (Close all positions & stop system)\n\n"
                    "🔧 **4. Maintenance & AI Brain Sync:**\n"
                    "• `/health` - Inspect VPS CPU, RAM, SQLite & Network vitals\n"
                    "• `/sync_brain` - Hot-reload 33 AI Models Swarm & Wall Street ML weights\n"
                    "════════════\n"
                ...
                admin_panel_card = (
                    ...
                    "• `/admin_license 123456789 30` - 授予或撤销 VIP 授权\n"
                    "• `/admin_users` - 查看完整注册用户名录与状态\n\n"
                    "🚨 **3. 信号与紧急操作：**\n"
                    "• `/admin_signal BTC BUY 10` - 广播自动跟随交易信号\n"
                    "• `/admin_broadcast 市场快讯` - 向所有用户发送紧急广播\n"
                    "• `/toggle_breaker` - 开启/关闭熔断开关\n"
                    "• `/toggle_rebalance` - 开启/关闭智能再平衡\n"
                    "• `/admin_reset_pin 123456789` - 重置用户 2FA PIN 码\n"
                    "• `/admin_delete 123456789` - 删除用户账户记录\n"
                    "• `/admin_nuke 1234` - 紧急一键平仓并关闭系统\n\n"
                    "🔧 **4. 节点维护与大脑同步：**\n"
                    "• `/health` - 检查 VPS 硬件指标、SQLite 与网络延迟\n"
                    "• `/sync_brain` - 热重载 33 AI 模型集群与华尔街 ML 权重\n"
                    "════════════\n"
                    "💡 _点击下方交互式按钮立即执行：_"
                )
            else:
                admin_panel_card = (
                    ...
                    "• `/admin_license 123456789 30` - ផ្តល់ ឬដក VIP Membership\n"
                    "• `/admin_users` - មើលបញ្ជីឈ្មោះសមាជិកចុះឈ្មោះទាំងអស់\n\n"
                    "🚨 **3. Signal & Emergency Operations ៖**\n"
                    "• `/admin_signal BTC BUY 10` - បាញ់ Signal ជួញដូរស្វ័យប្រវត្តិ\n"
                    "• `/admin_broadcast ដំណឹងទីផ្សារ` - ផ្ញើសារដំណឹងអាសន្នទៅកាន់គ្រប់ User\n"
                    "• `/toggle_breaker` - បើក/បិទ ស្វិតអាសន្ន Circuit Breaker\n"
                    "• `/toggle_rebalance` - បើក/បិទ យន្តការ Capital Rebalance\n"
                    "• `/admin_reset_pin 123456789` - កំណត់ PIN 2FA ឡើងវិញជូន User\n"
                    "• `/admin_delete 123456789` - លុបទិន្នន័យ User ចេញពីប្រព័ន្ធ\n"
                    "• `/admin_nuke 1234` - បិទ និង Market Close គ្រប់ Positions ទាំងអស់អាសន្ន\n\n"
                    "🔧 **4. Maintenance & AI Brain Sync ៖**\n"
                    "• `/health` - ពិនិត្យសុខភាពម៉ាស៊ីន VPS, CPU, RAM, SQLite & Network\n"
                    "• `/sync_brain` - Hot-reload ទម្ងន់ 33 AI Models Swarm & Wall Street ML\n"
                    "════════════\n"
        ...
        async def top_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            try:
                ...
                ai_prompt = (
                    ...
                    f"`/turbo_hedge TOP 20 10 BUY 5 1234`\n\n"
        ...
        async def master_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if data == "btn_menu_refresh":
                ...
                context.args = ["GOLD", "20", "10", "AUTO"]
                await smart_x_command(update, context)
            elif data == "btn_smart_x_spot":
                context.args = ["SPOT", "50"]
                await smart_x_command(update, context)
            elif data == "btn_smart_x_futures":
                context.args = ["GOLD", "20", "10", "AUTO"]
                await smart_x_command(update, context)
            elif data == "btn_smart_x_auto":
                context.args = ["AUTO", "20", "10"]
                await smart_x_command(update, context)
            elif data == "btn_smart_x_radar":
                context.args = ["RADAR"]
                await smart_x_command(update, context)
                ...
                await smart_swap_command(update, context)
            elif data == "btn_smart_swap_bind_prompt":
                context.args = ["BIND_PROMPT"]
                await smart_swap_command(update, context)
            elif data == "btn_smart_swap_withdraw_phantom":
                context.args = ["WITHDRAW", "ALL", "PHANTOM", "1234"]
                await smart_swap_command(update, context)
                ...
                await toggle_rebalance_command(update, context)
            elif data in ["btn_reset_pin_prompt", "btn_reset_pin"]:
                context.args = []
                await reset_pin_command(update, context)
            elif data == "btn_pin_change_guide":
                guide_txt = (
                    "🔄 **HOW TO CHANGE YOUR 2FA PIN ៖**\n\n"
                    "Please send this command in chat:\n"
                    "`` `/reset_pin <OLD_PIN> <NEW_PIN>` ``\n\n"
                    "*(Example: `` `/reset_pin 1234 5678` ``)*\n\n"
                    "💡 _Your PIN message will be automatically deleted from Telegram chat immediately!_"
                )
                await query.message.reply_text(guide_txt, parse_mode="Markdown")
            elif data == "btn_pin_forgot":
                context.args = ["forgot"]
                await reset_pin_command(update, context)
            elif data == "btn_pin_status":
                u_pin = db.get_user_pin(chat_id)
                ...
                await admin_reset_pin_command(update, context)
            elif data == "btn_pin_request_ticket":
                from datetime import datetime, timezone
                user_obj = update.effective_user
                u_name = f"@{user_obj.username}" if (user_obj and user_obj.username) else f"User {chat_id}"
                is_vip_st = "👑 VIP Elite User" if db.is_vip(chat_id) else "Registered User"
                t_stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
                ...
                admin_kb = InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("✅ Approve & Auto-Gen PIN", callback_data=f"btn_pin_approve_{chat_id}"),
                        InlineKeyboardButton("❌ Reject Request", callback_data=f"btn_pin_reject_{chat_id}")
                    ]
                ])
                try:
                    ...
                    await query.message.reply_text(f"⚠️ Could not reach Super Admin: {e}", parse_mode="Markdown")
            elif data.startswith("btn_pin_approve_"):
                if not (chat_id == 859271875 or db.is_admin(chat_id)):
                    await query.answer("⛔ Super Admin Only!", show_alert=True)
                else:
                    target_id_str = data.replace("btn_pin_approve_", "").strip()
                    if target_id_str.isdigit():
                        t_id = int(target_id_str)
                        import secrets
                        tmp_pin = f"{secrets.randbelow(900000) + 100000}"
                        p_hash = security.hash_pin(tmp_pin, t_id)
                        db.set_user_pin(t_id, p_hash)
                        self.failed_pin_tracker.pop(t_id, None)
                        if hasattr(db, 'log_admin_action'):
                            db.log_admin_action(chat_id, "RESET_PIN_TICKET_APPROVED", str(t_id), f"Auto-generated PIN {tmp_pin}")
                        ...
                        try:
                            ...
                            print(f"Failed to notify user {t_id}: {err_notif}")

                        await query.edit_message_text(
                            f"✅ **APPROVED & DISPATCHED!** 🟢\n\n• Target User: `{t_id}`\n• Temporary PIN: `{tmp_pin}`\n• User notified directly via Telegram DM.",
                            parse_mode="Markdown"
                        )
            elif data.startswith("btn_pin_reject_"):
                if not (chat_id == 859271875 or db.is_admin(chat_id)):
                    await query.answer("⛔ Super Admin Only!", show_alert=True)
                else:
                    target_id_str = data.replace("btn_pin_reject_", "").strip()
                    if target_id_str.isdigit():
                        t_id = int(target_id_str)
                        if hasattr(db, 'log_admin_action'):
                            db.log_admin_action(chat_id, "RESET_PIN_TICKET_REJECTED", str(t_id), "Admin rejected reset request")
                        try:
                            await context.bot.send_message(
                                chat_id=t_id,
                                text="❌ **PIN Reset Request Denied**: Your emergency reset request was declined by Super Admin for security verification.",
                                parse_mode="Markdown"
                            )
                        except Exception:
                            pass
                        await query.edit_message_text(f"❌ **REJECTED**: PIN reset request for User `{t_id}` declined.", parse_mode="Markdown")
        ...
        async def reset_pin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            """Super Smart 2FA PIN Manager, Recovery & Reset Engine."""
            if not await verify_user(update): return
            chat_id = update.effective_chat.id if update.effective_chat else (update.callback_query.message.chat.id if update.callback_query and update.callback_query.message else None)
            if not chat_id: return
            ...
            import time
            now = time.time()
            tracker = self.failed_pin_tracker.get(chat_id, {"count": 0, "locked_until": 0.0})
            if tracker.get("locked_until", 0.0) > now:
                rem_sec = int(tracker["locked_until"] - now)
                rem_min = max(1, rem_sec // 60)
                if user_lang == 'en':
                    lock_msg = f"⛔ **SECURITY LOCKOUT ACTIVE**\n\nToo many failed attempts. PIN actions locked for `{rem_min} minutes` for security."
                elif user_lang == 'zh':
                    lock_msg = f"⛔ **安全锁定期激活**\n\n失败次数过多。为保护账户安全，PIN 码操作已锁定 `{rem_min} 分钟`。"
                else:
                    lock_msg = f"⛔ **គណនីត្រូវបានចាក់សោសុវត្ថិភាពបណ្តោះអាសន្ន**\n\nដោយសារមានការព្យាយាមខុសច្រើនដង។ ប្រព័ន្ធបានចាក់សោ `{rem_min} នាទី` ដើម្បីការពារសុវត្ថិភាព!"
                if update.callback_query:
                    await update.callback_query.message.reply_text(lock_msg, parse_mode="Markdown")
                else:
                    await update.effective_message.reply_text(lock_msg, parse_mode="Markdown")
                return
            ...
            existing_pin = db.get_user_pin(chat_id)
            pin_status_str = "🟢 Active (សកម្ម)" if existing_pin else "🔴 Not Configured (មិនទាន់កំណត់)"

            keyboard_rows = [
                [
                    InlineKeyboardButton("🔄 Change PIN (ប្តូរលេខ PIN)", callback_data="btn_pin_change_guide"),
                    InlineKeyboardButton("🆘 Forgot PIN? (ភ្លេចលេខកូដ?)", callback_data="btn_pin_forgot")
                ],
                [
                    InlineKeyboardButton("🛡️ Security Status", callback_data="btn_pin_status"),
                    InlineKeyboardButton("🎛️ Master Menu", callback_data="btn_menu_refresh")
                ]
            ]
            if is_admin_user:
                keyboard_rows.insert(1, [
                    InlineKeyboardButton("👑 Admin Reset User PIN", callback_data="btn_admin_reset_pin_prompt")
                ])
            keyboard = InlineKeyboardMarkup(keyboard_rows)

            # Case 1: No arguments -> Interactive Super Smart Security Dashboard
            if len(args) == 0:
                if user_lang == 'en':
                    card = (
                        ...
                        "• **Anti-Brute-Force Shield**: `5-Attempt Max / 30m Auto-Lockout` 🛡️\n"
                        "• **Auto-Purge Destruction**: `Sub-Second PIN Chat Erase (< 500ms)` ⚡\n\n"
                        "📋 **QUICK COMMAND SYNTAX:**\n"
                        "👉 **1. Change PIN (With Current PIN):**\n"
                        "`` `/reset_pin <OLD_PIN> <NEW_PIN>` ``\n\n"
                        "👉 **2. Forgot PIN (Zero-Knowledge Self-Recovery):**\n"
                        "`` `/reset_pin verify <LAST_4_API_KEY> <NEW_PIN>` ``\n"
                        "*(or tap 🆘 Forgot PIN below for 1-Tap Admin Ticket)*\n\n"
                    )
                    if is_admin_user:
                        card += (
                            "👑 **SUPER ADMIN OVERRIDE SYNTAX:**\n"
                            "• Reset Self: `` `/reset_pin <NEW_PIN>` ``\n"
                    ...
                    card = (
                        ...
                        "• **防爆破护盾**: `最多 5 次尝试 / 30分钟锁定` 🛡️\n"
                        "• **敏感信息自毁**: `毫秒级 Telegram 聊天记录秒删 (< 500ms)` ⚡\n\n"
                        "📋 **快捷操作格式：**\n"
                        "👉 **1. 修改 PIN 码 (已知旧密码)：**\n"
                        "`` `/reset_pin <旧PIN> <新PIN>` ``\n\n"
                        "👉 **2. 忘记 PIN 码 (零知识自主找回)：**\n"
                        "`` `/reset_pin verify <API_KEY后4位> <新PIN>` ``\n"
                        "*(或点击下方 🆘 忘记密码提交一键工单)*\n\n"
                    )
                    if is_admin_user:
                        card += (
                    ...
                    card = (
                        ...
                        f"• **Current PIN Vault** ៖ `{pin_status_str}`\n"
                        "• **Encryption Vault** ៖ `PBKDF2 Multi-Layer Cryptographic Salt` 🟢\n"
                        "• **Anti-Brute-Force Shield** ៖ `៥ ដងអតិបរមា / ចាក់សោ ៣០ នាទី` 🛡️\n"
                        "• **Auto-Purge Destruction** ៖ `លុបសារចេញពី Chat ភ្លាមៗ (< 500ms)` ⚡\n\n"
                        "📋 **QUICK COMMAND SYNTAX (ទម្រង់ពាក្យបញ្ជា) ៖**\n"
                        "👉 **១. ប្តូរលេខ PIN (ស្គាល់ PIN ចាស់) ៖**\n"
                        "`` `/reset_pin <OLD_PIN> <NEW_PIN>` ``\n\n"
                        "👉 **២. ភ្លេចលេខ PIN (Zero-Knowledge Self-Recovery) ៖**\n"
                        "`` `/reset_pin verify <កន្ទុយ_API_៤_ខ្ទង់> <NEW_PIN>` ``\n"
                        "*(ឬចុចប៊ូតុង 🆘 ភ្លេចលេខកូដ? ខាងក្រោមដើម្បីផ្ញើសំណើទៅកាន់ Admin)*\n\n"
                    )
                    if is_admin_user:
                        card += (
                            "👑 **SUPER ADMIN OVERRIDE (សិទ្ធិពិសេស ADMIN) ៖**\n"
                            "• Reset ខ្លួនឯង ៖ `` `/reset_pin <NEW_PIN>` ``\n"
                ...
                if update.callback_query:
                    try:
                        await update.callback_query.edit_message_text(card, parse_mode="Markdown", reply_markup=keyboard)
                    except Exception:
                        await update.callback_query.message.reply_text(card, parse_mode="Markdown", reply_markup=keyboard)
                else:
                    await update.effective_message.reply_text(card, parse_mode="Markdown", reply_markup=keyboard)
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                return
            ...
            if len(args) == 1:
                arg0 = args[0].lower()
                # Sub-case 2.1: Forgot / Reset Request
                if arg0 in ["forgot", "lost", "help", "recover", "reset"]:
                    user_api_key, _ = db.get_user_api(chat_id)
                    has_api = bool(user_api_key and len(user_api_key) >= 4)

                    forgot_kb = InlineKeyboardMarkup([
                        [InlineKeyboardButton("📨 Request Admin Reset Ticket", callback_data="btn_pin_request_ticket")],
                        [InlineKeyboardButton("🎛️ Back to Menu", callback_data="btn_menu_refresh")]
                    ])
                    ...
                    if user_lang == 'en':
                        ...
                        if has_api:
                            forgot_card += (
                                "👉 **METHOD 1: Instant Zero-Knowledge API Verification**\n"
                                "Verify ownership using the last 4 characters of your connected Binance API Key:\n"
                                "`` `/reset_pin verify <LAST_4_API_KEY> <NEW_PIN>` ``\n\n"
                            )
                        forgot_card += (
                        ...
                        if has_api:
                            forgot_card += (
                                "👉 **ជម្រើសទី ១ ៖ Zero-Knowledge API Verification (លឿនបំផុត)**\n"
                                "ផ្ទៀងផ្ទាត់ដោយប្រើលេខកន្ទុយ ៤ ខ្ទង់ចុងក្រោយនៃ Binance API Key របស់អ្នក ៖\n"
                                "`` `/reset_pin verify <កន្ទុយ_៤_ខ្ទង់> <NEW_PIN>` ``\n\n"
                            )
                        forgot_card += (
                    ...
                    if update.callback_query:
                        await update.callback_query.edit_message_text(forgot_card, parse_mode="Markdown", reply_markup=forgot_kb)
                    else:
                        await update.effective_message.reply_text(forgot_card, parse_mode="Markdown", reply_markup=forgot_kb)
                        await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    return

                # Sub-case 2.2: Admin resetting their OWN PIN directly
                if is_admin_user and args[0].isdigit():
                    new_pin = args[0]
                    valid, reason = is_smart_pin(new_pin)
                    if not valid:
                        await update.effective_message.reply_text(f"❌ **{reason}**", parse_mode="Markdown")
                    ...
                    pin_hash = security.hash_pin(new_pin, chat_id)
                    db.set_user_pin(chat_id, pin_hash)
                    self.failed_pin_tracker.pop(chat_id, None)
                    if hasattr(db, 'log_admin_action'):
                        db.log_admin_action(chat_id, "RESET_PIN_SELF", str(chat_id), "Admin directly reset self PIN")
                    ...
                    await update.effective_message.reply_text(success_msg, parse_mode="Markdown", reply_markup=keyboard)
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    self.log_signal.emit(f"👑 Super Admin {chat_id} updated their PIN.")
                    return

                # Sub-case 2.3: User provided single PIN but has existing PIN
                if existing_pin:
                    err_exist = (
                        "⚠️ **You already have a PIN set!**\n\n"
                        "• To change your PIN: `` `/reset_pin <OLD_PIN> <NEW_PIN>` ``\n"
                        "• If you forgot your PIN: `` `/reset_pin forgot` ``" if user_lang == 'en' else
                        ("⚠️ **您已设置过 PIN 码！**\n\n"
                         "• 修改密码： `` `/reset_pin <旧PIN> <新PIN>` ``\n"
                         "• 忘记密码： `` `/reset_pin forgot` ``" if user_lang == 'zh' else
                         "⚠️ **អ្នកបានកំណត់លេខកូដ PIN រួចហើយ!**\n\n"
                         "• ដើម្បីប្តូរលេខ PIN ៖ `` `/reset_pin <OLD_PIN> <NEW_PIN>` ``\n"
                         "• ប្រសិនបើភ្លេចលេខ PIN ៖ `` `/reset_pin forgot` ``")
                    )
                    await update.effective_message.reply_text(err_exist, parse_mode="Markdown", reply_markup=keyboard)
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    return
                else:
                    new_pin = args[0]
                    ...
                    await update.effective_message.reply_text(success_msg, parse_mode="Markdown", reply_markup=keyboard)
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    self.log_signal.emit(f"🔒 User {chat_id} set their 2FA security PIN.")
                    return

            # Case 3: len(args) == 2
            if len(args) == 2:
                # Sub-case 3.1: Admin resetting another user: /reset_pin <USER_ID> <NEW_PIN>
                if is_admin_user and args[0].isdigit() and (int(args[0]) != chat_id or len(args[0]) > 6):
                    ...
                    self.failed_pin_tracker.pop(target_id, None)

                    if hasattr(db, 'log_admin_action'):
                        db.log_admin_action(chat_id, "RESET_PIN_USER", str(target_id), f"Admin reset PIN for {target_id}")
                    ...
                    admin_rep = (
                        ...
                        "• **Direct Dispatch**: `Sent private alert to user Telegram`\n\n"
                        "💡 _Audit event recorded in security logs._"
                    )
                    await update.effective_message.reply_text(admin_rep, parse_mode="Markdown", reply_markup=keyboard)
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    return

                # Sub-case 3.2: User changing PIN: /reset_pin <OLD_PIN> <NEW_PIN>
                old_pin = args[0]
                new_pin = args[1]

                if existing_pin and not security.verify_pin(old_pin, chat_id, existing_pin):
                    t = self.failed_pin_tracker.get(chat_id, {"count": 0, "locked_until": 0.0})
                    t["count"] += 1
                    remaining = 5 - t["count"]
                    if t["count"] >= 5:
                        t["locked_until"] = now + 1800.0
                        lock_err = "⛔ **SECURITY LOCKOUT**: 5 failed PIN attempts! PIN operations locked for 30 minutes."
                        await update.effective_message.reply_text(lock_err, parse_mode="Markdown")
                    else:
                        bad_old = (
                            f"❌ **Incorrect Old PIN!** ({remaining} attempts remaining before 30m lockout)\n\n"
                            "💡 If you forgot your PIN, use: `` `/reset_pin forgot` ``" if user_lang == 'en' else
                            (f"❌ **旧 PIN 码不正确！** (还剩 {remaining} 次机会即锁定 30 分钟)\n\n"
                             "💡 如果忘记密码，请使用： `` `/reset_pin forgot` ``" if user_lang == 'zh' else
                             f"❌ **លេខកូដ PIN ចាស់មិនត្រឹមត្រូវ!** (នៅសល់ {remaining} ដងទៀត មុននឹងត្រូវចាក់សោ ៣០ នាទី)\n\n"
                             "💡 ប្រសិនបើភ្លេចលេខ PIN សូមប្រើ ៖ `` `/reset_pin forgot` ``")
                        )
                        await update.effective_message.reply_text(bad_old, parse_mode="Markdown")
                    self.failed_pin_tracker[chat_id] = t
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    return
                ...
                await update.effective_message.reply_text(success_msg, parse_mode="Markdown", reply_markup=keyboard)
                await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                self.log_signal.emit(f"🔒 User {chat_id} changed their security PIN.")
                return

            # Case 4: len(args) == 3 and args[0].lower() == "verify"
            if len(args) == 3 and args[0].lower() == "verify":
                api_fingerprint = args[1]
                new_pin = args[2]
                ...
                if not user_api_key:
                    ...
                    if bybit_keys:
                        user_api_key = bybit_keys[0]

                if not user_api_key or len(user_api_key) < 4:
                    no_api_err = (
                        "❌ **Zero-Knowledge Recovery Unavailable**: No connected API key found in your account.\n\n"
                        "👉 Please use: `` `/reset_pin forgot` `` to submit an Emergency Admin Ticket!"
                    )
                    await update.effective_message.reply_text(no_api_err, parse_mode="Markdown")
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    return

                if not user_api_key.endswith(api_fingerprint):
                    t = self.failed_pin_tracker.get(chat_id, {"count": 0, "locked_until": 0.0})
                    t["count"] += 1
                    self.failed_pin_tracker[chat_id] = t
                    mismatch_err = "❌ **Verification Failed**: Last 4 characters do not match your active API key."
                    await update.effective_message.reply_text(mismatch_err, parse_mode="Markdown")
                    await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                    return
                ...
                if hasattr(db, 'log_admin_action'):
                    db.log_admin_action(chat_id, "RESET_PIN_ZK", str(chat_id), "Self-recovered PIN via Zero-Knowledge API verification")
                ...
                await update.effective_message.reply_text(success_msg, parse_mode="Markdown", reply_markup=keyboard)
                await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
                self.log_signal.emit(f"🎉 User {chat_id} self-recovered PIN via Zero-Knowledge proof.")
                return

            await update.effective_message.reply_text("📋 Syntax: `/reset_pin <OLD_PIN> <NEW_PIN>` or `/reset_pin forgot`", parse_mode="Markdown")
            await delete_sensitive_message(context, chat_id, update.effective_message.message_id, user_lang)
            return
        ...
        async def group_id_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if chat_type in ["group", "supergroup", "channel"]:
                msg = (
                    "💎 **TELEGRAM GROUP / CHANNEL ID RADAR** 📡\n"
                    "════════════\n"
                    f"🏷️ **ឈ្មោះ Group** ៖ `{chat_title}`\n"
                    f"🆔 **Group ID** ៖ `{chat_id}`\n"
        ...
        async def set_alert_group_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            if not await verify_user(update): return
            chat_id = update.effective_chat.id if update.effective_chat else None
            if not (chat_id == 859271875 or db.is_admin(chat_id)):
            ...
            if not raw_args or len(raw_args) < 2:
                if msg_target:
                    await msg_target.reply_text(
                        ...
                        "🛡️ **Delta-Neutral Hedge (Auto)** ៖ `` `/turbo_hedge HEDGE auto 20 1234` ``\n"
                        "🛡️ **Delta-Neutral Single Coin** ៖ `` `/turbo_hedge HEDGE BTC 100 1234` ``\n"
            ...
            if not args or len(args) == 0:
                ...
                if user_lang == 'en':
                    ...
                    msg = (
                        ...
                        "• 🚀 **TURBO Sprint** ៖ `XAUUSDT` (Dynamic 25x-50x, Asymmetric 5R-6R Engine: 25% TP1 Cash + 75% Moonbag Runner)\n"
                        "• 🛡️ **SONIC Scalp** ៖ `XAUUSDT` (Leverage 10x-20x, 87.12% Win Rate, Asymmetric 5.5R Risk/Reward, Golden 85% Ratchet)\n"
            ...
            if is_spot:
                ...
                if mode_label == "TURBO":
                    ...
                    ack_text = (
                        ...
                        f"  • Profit Target: `Asymmetric 5R-6R (25% TP1 Cash + 75% Moonbag Runner)`\n"
                        f"  • Risk Shield  : `Breakeven Armor @ +1.5R (+$0.75) | Golden 85% Ratchet`\n"
                        f"{div}\n"
                    ...
                    ack_text = (
                        ...
                        f"🛡️ *Risk Shield*  : `Asymmetric 5R-6R + Golden 85% Ratchet Armor`\n"
                        f"{div}\n"
                        ...
                        if sig.get("side") in ["BUY", "SELL"]:
                            ...
                            if exec_res.get("status") in ["success", "NEW", "FILLED"] or exec_res.get("orderId"):
                                ...
                                notif = (
                                    ...
                                    f"• Target TP  : `Asymmetric 5R-6R (25% TP1 Cash + 75% Moonbag Runner)`\n"
                                    f"• Protection : `Breakeven Armor @ +1.5R (+$0.75) | Golden 85% Ratchet`\n"
                                    f"{div}\n"
        ...
        async def smart_swap_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if subcmd in ["WALLET", "HOTWALLET", "BOTWALLET"]:
                ...
                phantom_vault = solana_trading_wallet.get_user_phantom_wallet(chat_id)
                if phantom_vault:
                    vault_display = f"`{phantom_vault}`\n*(🟢 ភ្ជាប់រួចរាល់ - Auto Profit Settlement Vault)*"
                    kb_buttons = [
                        [
                            InlineKeyboardButton("🔄 ពិនិត្យសមតុល្យ (Refresh)", callback_data="btn_smart_swap_refresh_wallet"),
                            InlineKeyboardButton("💸 ដកចូល Phantom ភ្លាមៗ", callback_data="btn_smart_swap_withdraw_phantom")
                        ],
                        [
                            InlineKeyboardButton("🔗 ប្តូរ Phantom Wallet", callback_data="btn_smart_swap_bind_prompt"),
                    ...
                    vault_display = "`⚠️ មិនទាន់ភ្ជាប់ (Not Linked)`\n*(ចុចប៊ូតុង '🔗 ភ្ជាប់ Phantom Wallet' ខាងក្រោម)*"
                    kb_buttons = [
                        [
                            InlineKeyboardButton("🔄 ពិនិត្យសមតុល្យ (Refresh)", callback_data="btn_smart_swap_refresh_wallet"),
                            InlineKeyboardButton("🔗 ភ្ជាប់ Phantom Wallet", callback_data="btn_smart_swap_bind_prompt")
                        ],
                        [
                            InlineKeyboardButton("💸 ដកប្រាក់ SOL (Custom)", callback_data="btn_smart_swap_withdraw_prompt"),
                            InlineKeyboardButton("🔑 យក Private Key (Export)", callback_data="btn_smart_swap_export_prompt")
                        ],
                        [
                            InlineKeyboardButton("⚡ Auto Gem Sniper ($20)", callback_data="btn_smart_swap_auto_20")
                ...
                wallet_card = (
                    ...
                    f"📍 **កាបូប Bot Trading Wallet (សម្រាប់ដាក់ទុនជួញដូរ) ៖**\n"
                    f"`{pub}`\n\n"
                    f"🏦 **កាបូប Phantom Profit Vault (សម្រាប់ទទួលប្រាក់ចំណេញ) ៖**\n"
                    f"{vault_display}\n\n"
                    ...
                    f"────────────\n"
                    f"💡 **របៀបប្រើប្រាស់ SUPER SMART ៖**\n"
                    f"1. ផ្ញើ **SOL** ពី Phantom របស់អ្នកមកកាន់កាបូបជួញដូរ `{pub}`\n"
                    f"2. វាយ `/smart_swap auto 20 1234` ដើម្បីឱ្យ Bot ទិញកាក់ Gem On-Chain អូតូ\n"
                    f"3. ចុចប៊ូតុង **'💸 ដកចូល Phantom ភ្លាមៗ'** ដើម្បីផ្ញើប្រាក់ចំណេញទៅកាន់ Phantom របស់អ្នកវិញដោយសុវត្ថិភាព ១០០%!"
                )
            ...
            if subcmd in ["WITHDRAW", "SEND_SOL"]:
                if len(args) < 2:
                    help_msg = (
                        ...
                        "────────────\n\n"
                        "📌 **ទម្រង់បញ្ជា ៖**\n"
                        "• ដកចូល Phantom ផ្ទាល់ខ្លួន ៖ `/smart_swap withdraw all phantom 1234`\n"
                        "• ដកទៅកាបូបជាក់លាក់ ៖ `/smart_swap withdraw <ចំនួន_SOL> <កាបូប_Solana_ទទួល> <PIN>`\n\n"
                        "💡 **ឧទាហរណ៍ ៖**\n"
                        "• ដកទាំងអស់ចូល Phantom ៖ `/smart_swap withdraw all`\n"
                        "• ដក 0.5 SOL ចូល Phantom ៖ `/smart_swap withdraw 0.5`\n"
                        "• ដកទៅកាបូបផ្សេង ៖ `/smart_swap withdraw 0.5 9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin 1234`"
                    )
                    if msg_target:
                ...
                dest_addr = str(args[2]).strip() if len(args) > 2 else "PHANTOM"
                pin_input = str(args[3]).strip() if len(args) > 3 else "1234"

                # If 2nd arg was actually a PIN (e.g., /smart_swap withdraw all 1234)
                if dest_addr.isdigit() and len(dest_addr) == 4:
                    pin_input = dest_addr
                    dest_addr = "PHANTOM"
            ...
            # 0.3 BIND_PHANTOM (Connect Personal Phantom Address as Profit Settlement Vault)
            if subcmd in ["BIND_PHANTOM", "BIND", "CONNECT_PHANTOM"]:
                if len(args) < 2:
                    help_bind = (
                        "🔗 **APEX SMART SWAP | ភ្ជាប់ PHANTOM WALLET ផ្ទាល់ខ្លួន**\n"
                        "────────────\n\n"
                        "📌 **ទម្រង់បញ្ជា ៖**\n"
                        "`/smart_swap bind_phantom <អាសយដ្ឋាន_Solana_របស់អ្នក>`\n\n"
                        "💡 **ឧទាហរណ៍ ៖**\n"
                        "`/smart_swap bind_phantom 9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin`"
                    )
                    if msg_target:
                        await msg_target.reply_text(help_bind, parse_mode="Markdown")
                    return

                target_addr = str(args[1]).strip()
                import solana_trading_wallet
                bind_res = solana_trading_wallet.bind_user_phantom_wallet(chat_id, target_addr)
                if bind_res.get("status") == "success":
                    bind_card = (
                        "🎉 **PHANTOM WALLET LINKED SUCCESSFULLY!** 🔗\n"
                        "────────────\n\n"
                        f"🏦 **Phantom Settlement Vault ៖**\n"
                        f"`{target_addr}`\n\n"
                        "✅ **កាបូប Phantom ផ្ទាល់ខ្លួនរបស់អ្នកត្រូវបានចងភ្ជាប់ធ្វើជា Profit Settlement Vault រួចរាល់!**\n"
                        "• 🛡️ សុវត្ថិភាពខ្ពស់បំផុត ១០០% (Bot ស្គាល់ត្រឹមតែអាសយដ្ឋានទទួលលុយ គ្មាន Private Key)\n"
                        "• 💸 រាល់ពេលដកប្រាក់ ឬកើបផលចំណេញ លោកអ្នកគ្រាន់តែចុច '💸 ដកចូល Phantom ភ្លាមៗ' ជាការស្រេច!"
                    )
                    kb = InlineKeyboardMarkup([
                        [
                            InlineKeyboardButton("💳 មើលកាបូប /smart_swap", callback_data="btn_smart_swap_wallet"),
                            InlineKeyboardButton("⚡ Auto Gem Sniper ($20)", callback_data="btn_smart_swap_auto_20")
                        ]
                    ])
                    if msg_target:
                        await msg_target.reply_text(bind_card, reply_markup=kb, parse_mode="Markdown")
                else:
                    if msg_target:
                        await msg_target.reply_text(f"❌ **ការភ្ជាប់បរាជ័យ ៖** {bind_res.get('msg', 'Invalid Address')}")
                return

            if subcmd == "BIND_PROMPT":
                prompt_msg = (
                    "🔗 **APEX SMART SWAP | ភ្ជាប់ PHANTOM WALLET ផ្ទាល់ខ្លួន (PROFIT VAULT)**\n"
                    "────────────\n\n"
                    "💡 *កាបូប Phantom ផ្ទាល់ខ្លួនរបស់អ្នក នឹងដើរតួជាកន្លែងទទួលប្រាក់ចំណេញ (Profit Settlement Vault) ដោយសុវត្ថិភាព ១០០% (Zero Risk - មិនត្រូវការ Private Key ឡើយ)!*\n\n"
                    "📌 **របៀបភ្ជាប់ងាយៗ (១ ជំហាន) ៖**\n"
                    "សូម Copy និង Paste ពាក្យបញ្ជាខាងក្រោម រួចប្តូរអាសយដ្ឋានរបស់អ្នក ៖\n\n"
                    "`/smart_swap bind_phantom <អាសយដ្ឋាន_Solana_របស់អ្នក>`\n\n"
                    "💡 **ឧទាហរណ៍ ៖**\n"
                    "`/smart_swap bind_phantom 9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin`\n\n"
                    "👉 **របៀប Copy អាសយដ្ឋានពី Phantom App ៖**\n"
                    "1. បើកកម្មវិធី **Phantom Wallet** លើទូរស័ព្ទរបស់អ្នក\n"
                    "2. ចុចលើពាក្យ **Receive** -> ជ្រើសរើស **Solana**\n"
                    "3. ចុច **Copy** អាសយដ្ឋាន រួចយកមក Paste ផ្ញើមកកាន់ Bot ឥឡូវនេះ!"
                )
                if msg_target:
            ...
            if subcmd == "WITHDRAW_PROMPT":
                prompt_msg = (
                    ...
                    "────────────\n\n"
                    "📌 **ទម្រង់បញ្ជាដកប្រាក់ ៖**\n"
                    "• ដកចូល Phantom ផ្ទាល់ខ្លួន ៖ `/smart_swap withdraw all`\n"
                    "• ដកចំនួនកំណត់ ៖ `/smart_swap withdraw 0.2`\n"
                    "• ដកទៅកាបូបផ្សេង ៖ `/smart_swap withdraw 0.2 <កាបូប_Solana> 1234`"
                )
                if msg_target:
                    await msg_target.reply_text(prompt_msg, parse_mode="Markdown")
                return
...
# import asyncio # removed local shadowing
        ...
        async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if turbo_cfg and turbo_cfg.get("is_enabled"):
                ...
                inactive_engines.append("👉 **Apex Turbo Hedge Engine ៖**\n`` `/turbo_hedge BTCUSDT 20 10 BUY 2.5 1234` ``")
            ...
            # 5. Pre-Pump Spike Sniper
            if snipe_cfg and snipe_cfg.get("is_enabled"):
                amt = snipe_cfg.get("amount", 50.0)
                active_engines.append(f"• 🎯 **Pre-Pump Spike Sniper**: `ACTIVE` (Amount: `${amt}`)")
            else:
                inactive_engines.append("👉 **Pre-Pump Spike Sniper ៖**\n`` `/pre_pump ON 50` ``")
        ...
        self.app.add_handler(CommandHandler("group_id", group_id_command))
        self.app.add_handler(CommandHandler("groupid", group_id_command))
        ...
        self.app.add_handler(CommandHandler("reset_pin", reset_pin_command))
        ...
        async def paper_trading_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if not args or len(args) == 0:
                ...
                if user_lang == 'en':
                    msg = (
                        ...
                        "`` `/paper_trading OFF 1234` ``\n\n"
                        "👉 **Switch to SAFE PAPER TRADING SIMULATION:**\n"
                        "`` `/paper_trading ON 1234` ``\n"
                        "════════════\n"
            ...
            if user_pin and not security.verify_pin(input_pin, chat_id, user_pin):
                bad_pin = "❌ Security Error: Invalid 2FA PIN! Usage: `/paper_trading OFF 1234`" if user_lang == 'en' else ("❌ 安全错误：2FA PIN 码不正确！格式：`/paper_trading OFF 1234`" if user_lang == 'zh' else "❌ កំហុសសុវត្ថិភាព ៖ លេខកូដ 2FA PIN មិនត្រឹមត្រូវ! (ទម្រង់ ៖ `/paper_trading OFF 1234`)")
        ...
        # 1c. Super Smart /auto_trade Macro Waterfall & Breakout Monitor (Every 15 seconds)
        self.scheduler.add_job(
            scheduler_tasks.macro_auto_trade_monitor,
            'interval',
            seconds=15,
            max_instances=2,
            coalesce=True,
            args=[self.app],
            id='macro_auto_trade_monitor'
        )
