        ...
        if not ef or ef in ["turbo_hedge", "smart_trade", "smart_x"]:
            try:
                ...
                for inc in b_incomes:
                    ...
                    sym_inc = str(inc.get("symbol", "")).upper()
                    target_eng = "smart_x" if ("PAXG" in sym_inc or "XAU" in sym_inc) else "turbo_hedge"

                    if ef and ef != target_eng:
                        continue
                    ...
                    if inc_type == "REALIZED_PNL":
                        ...
                        if inc_val >= 0:
                            wins += 1
                        engines[target_eng]["pnl"] += inc_val
                        engines[target_eng]["trades"] += 1
                        if inc_val >= 0:
                            engines[target_eng]["wins"] += 1
                ...
                for bt in b_trades[:5]:
                    bt_sym = str(bt.get("symbol", "BTCUSDT")).upper()
                    target_bt_eng = "smart_x" if ("PAXG" in bt_sym or "XAU" in bt_sym) else "turbo_hedge"
                    if ef and ef != target_bt_eng:
                        continue
                    ...
                    recent_trades.append({
                        ...
                        "qty": f"{b_qty:.3f} {bt_sym.replace('USDT','')}",
                        "entry_price": b_price,
                        "exit_price": b_price,
                        "time": b_time_str,
                        "pnl": b_pnl,
