...
ARBITRUM ONE FLASH LOAN SMART CONTRACT V3 (LIQUIDATION + MEV) DEPLOYMENT HELPER
================================================================================
Deploys SuperSmartFlashLoanArbitrageV3.sol to Arbitrum One Mainnet using Keeper Wallet.
Unlocks Balancer V2 0% fee flash loans, Aave V3 flash loans, and Aave V3 native
liquidation bounty extraction (liquidationCall with 5%-10% protocol bonus).
================================================================================
"""
...
def save_contract_to_env(contract_addr: str):
    ...
    if os.path.exists(env_path):
        ...
        if not found:
            new_lines.append(f"\n# SuperSmart Flash Loan Smart Contract V3 (Arbitrum One)\nFLASH_LOAN_CONTRACT_ADDRESS={contract_addr}\n")
...
def deploy_v3_arbitrum_contract() -> dict:
    ...
    print("=" * 75)
    print("  ARBITRUM ONE SUPERSMART FLASH LOAN V3 (LIQUIDATION) CONTRACT DEPLOYMENT")
    print("=" * 75)
    ...
    json_path = os.path.join(os.path.dirname(__file__), "SuperSmartFlashLoanArbitrageV3.json")
    if not os.path.exists(json_path):
        msg = f"Artifact not found: {json_path}. Compiling now..."
        print(f"⚙️ {msg}")
        import solcx
        solcx.install_solc('0.8.20')
        sol_path = os.path.join(os.path.dirname(__file__), "SuperSmartFlashLoanArbitrageV3.sol")
        with open(sol_path, 'r', encoding='utf-8') as f:
            src = f.read()
        compiled = solcx.compile_source(src, output_values=['abi', 'bin'], solc_version='0.8.20', optimize=True, optimize_runs=200)
        c_data = compiled['<stdin>:SuperSmartFlashLoanArbitrageV3']
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump({'contractName': 'SuperSmartFlashLoanArbitrageV3', 'abi': c_data['abi'], 'bytecode': c_data['bin']}, f, indent=2)
    ...
    if receipt.get("status") == 1:
        ...
        print("\n" + "🎉" * 35)
        print("  SUPER SMART FLASH LOAN V3 DEPLOYED SUCCESSFULLY ON ARBITRUM ONE!")
        print("🎉" * 35)
        ...
        deployed_record = {
            "contract_name": "SuperSmartFlashLoanArbitrageV3",
        ...
        rec_path = os.path.join(os.path.dirname(__file__), "SuperSmartFlashLoanArbitrageV3_deployed.json")
